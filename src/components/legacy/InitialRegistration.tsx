...
const translations = {
  ja: {
    ...
    studentIdErrTooLarge: '写真ファイルが大きすぎます。画質設定を下げるか、ギャラリーから圧縮済みの写真を選んでください。',
  ...
  en: {
    ...
    studentIdErrTooLarge: 'This photo file is too large. Lower your camera quality, or pick a smaller photo from your gallery.',
...
// バケット側の上限は 10MB。原本がこれを超える場合はカメラ画質を下げてもらう
...
export function InitialRegistration({ language, onLanguageChange, email, onComplete, onBack, existingUser }: InitialRegistrationProps) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const failWith = (message: string, code: string) => {
      toast.error(`${message}（${code}）`, { duration: 14000 });
    };

    const file = input.files?.[0];
    if (!file || file.size === 0) {
      failWith(t.studentIdErrNoPhoto, STUDENT_ID_ERROR_CODES.NO_PHOTO);
      clearInput();
      return;
    }
    if (!isProbablyImageFile(file)) {
      failWith(t.studentIdErrNotImage, STUDENT_ID_ERROR_CODES.NOT_IMAGE);
      clearInput();
      return;
    }
    if (file.size > MAX_STUDENT_ID_SIZE_MB * 1024 * 1024) {
      failWith(t.studentIdErrTooLarge, STUDENT_ID_ERROR_CODES.TOO_LARGE);
      clearInput();
      return;
    }
    ...
    try {
      ...
      if (message === 'HEIC_CONVERT_FAILED') {
        failWith(t.studentIdErrHeic, STUDENT_ID_ERROR_CODES.HEIC_CONVERT_FAILED);
      } else if (message === 'READ_FAILED' || /decode|tainted|security/i.test(message)) {
        failWith(t.studentIdErrReadFailed, STUDENT_ID_ERROR_CODES.READ_FAILED);
      } else {
        failWith(t.studentIdErrGeneric, STUDENT_ID_ERROR_CODES.GENERIC);
      }
    } finally {
  ...
  const validateForm = () => {
    ...
    if (normalizedStudentNumber !== formData.studentNumber) {
    ...
    setErrors(newErrors);
    if (normalizedStudentNumber && !validateStudentNumber(normalizedStudentNumber)) setStudentNumberError(t.studentNumberError);
    ...
    if (!normalizedStudentNumber) return toast.error(t.studentNumberRequired), scrollToError(studentNumberRef), false;
    if (!validateStudentNumber(normalizedStudentNumber)) return toast.error(t.studentNumberError), scrollToError(studentNumberRef), false;
    if (newErrors.phone)
      return (
        toast.error(
          language === "ja"
  ...
  const handleSubmit = () => {
    ...
    onComplete({
      name: formData.name,
      furigana: formData.furigana,
      studentNumber: normalizeStudentNumber(formData.studentNumber),
