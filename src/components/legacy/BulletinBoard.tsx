...
const translations = {
  ja: { title: '掲示板', subtitle: '言語交換・学習パートナーを見つけよう', createPost: '新しい投稿', postTitle: 'タイトル', postContent: '内容', tags: 'タグ（任意）', tagsHint: 'よく使うタグを選択、または追加', addCustomTag: 'カスタムタグを追加', peopleNeeded: '募集人数', uploadImage: '画像をアップロード', submit: '投稿する', cancel: 'キャンセル', languageExchange: '言語交換', studyGroup: '勉強会', event: 'イベント', displayType: '表示タイプ', storyDisplay: '1日のみ表示（ストーリー）', boardDisplay: 'ある期間まで表示（掲示板）', expiryDate: '表示期限', until: 'まで', replies: '件のリプライ', replyPlaceholder: 'リプライを入力...', sendReply: '返信', viewReplies: 'リプライを見る', addComment: 'コメントを追加...' },
  en: { title: 'Bulletin Board', subtitle: 'Find language exchange and study partners', createPost: 'New Post', postTitle: 'Title', postContent: 'Content', tags: 'Tags (optional)', tagsHint: 'Select common tags or add your own', addCustomTag: 'Add custom tag', peopleNeeded: 'People Needed', uploadImage: 'Upload Image', submit: 'Submit', cancel: 'Cancel', languageExchange: 'Language Exchange', studyGroup: 'Study Group', event: 'Event', displayType: 'Display Type', storyDisplay: '1 day only (Story)', boardDisplay: 'Display until date (Board)', expiryDate: 'Expiry Date', until: 'until', replies: 'replies', replyPlaceholder: 'Write a reply...', sendReply: 'Send', viewReplies: 'View replies', addComment: 'Add a comment...' }
};
const presetTags = { ja: ['English', '日本語', '中国語', '韓国語'], en: ['English', 'Japanese', 'Chinese', 'Korean'] };

export function BulletinBoard({ language, user, onInterested, boardPosts, onUpdateBoardPosts, onCreateBoardPost, onAddReply }: BulletinBoardProps) {
  ...
  const toggleTag = (tag: string) => setNewPost((prev) => ({ ...prev, tags: prev.tags.includes(tag) ? prev.tags.filter((t) => t !== tag) : [...prev.tags, tag] }));
  const addCustomTag = () => { if (customTagInput.trim() && !newPost.tags.includes(customTagInput.trim())) { setNewPost((prev) => ({ ...prev, tags: [...prev.tags, customTagInput.trim()] })); setCustomTagInput(''); setShowCustomTagInput(false); } };
  ...
  return (
    <div className="space-y-6">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2"><Label>{t.tags}</Label><div className="flex flex-wrap gap-1">{presetTags[language].filter((tag) => !newPost.tags.includes(tag)).map((tag) => <button key={tag} type="button" onClick={() => toggleTag(tag)} className="px-2 py-1 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs rounded-full transition-colors">{tag}</button>)}</div></div>
      ...
      {boardPostsList.length > 0 && <div className="space-y-4">{boardPostsList.map((post) => <BoardPostWithReplies key={post.id} post={post} language={language} user={user} onAddReply={handleAddReply} translations={t} />)}</div>}
