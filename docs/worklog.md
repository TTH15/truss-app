...
## 2026-08-11 13:40 運営画面に「設定」タブ新設 + 規約エディタの条文分割

- **設定タブ**: AdminPage に settings タブを追加（PC サイドバー + モバイル下部ナビの両方、歯車アイコン）。ヘッダー右上にあった通知設定（Bell ポップオーバー）と規約・ポリシー管理（巻物アイコンのモーダル）を廃止し、設定タブ内へ移動（`AdminSettings.tsx` 新設。ヘッダーは 会員画面へ/ログアウト のみに）
- **条文ごとの編集**: `AdminSiteDocuments` をダイアログからインラインパネルに変え、前文 + 条文（第N条 / N.）ごとの「見出し Input + 本文 textarea」に分割。条文の追加・削除も可能。保存時に1つの content に結合するため **DB スキーマは無変更**（履歴・保護・フォールバックもそのまま動く）
- **分割ロジック**（`splitSiteDocument` / `joinSiteDocument` @ lib/site-documents.ts): 文書スタイルを自動判定 — 「第N条」がある文書（規約）では「1. 」を条内の号として本文に残し、無い文書（ポリシー）では空行区切りブロック先頭の「N. 」を見出しとする。両文書でラウンドトリップ（分割→結合→再分割）の安定性を tsx で検証済み（条文数 13/13 一致）
- **公開ページの区切り表示**: `PolicyArticles` を新設し、前文 + 条文ごとの白カードで描画（公開ページと運営プレビューで共用）。`PolicyDocumentBody` に variant を追加し、条文本文内の「1. 」行が誤って大見出しにならないようにした
- 検証: `tsc` / `next build` 通過。lint エラーは AdminPage の既存3件のみ（今回未変更の effect）

## 2026-08-11 14:00 セッション総括と次セッションへの引き継ぎ（/clear 前の記録）

2026-08-10〜11 の「オンボーディング・ログイン整理」セッションの成果はすべて**コミット済み**（〜1c4da04）。migration・コミット・デプロイはユーザーが手動で進行中。

### このセッションで完成したもの（詳細は上の各エントリ参照）
1. **デッドコード除去とバグ修正**: 廃止メール認証の残骸6ファイル削除 / birthCountry 未保存バグ / 会費の巻き戻りバグ / fee_payment 案内の途切れ
2. **承認通知**: 承認時に本人へ運営名義メッセージ+プッシュ（日英併記）
3. **ログイン画面**: Google 公式風ボタン化・チェックボックスのポップアニメーション・説明文はユーザー指示で削除（SSR 側に審査用の説明・リンクは残存確認済み）
4. **URL**: /profile → /profile-setup（旧 URL は /dashboard へ redirect）
5. **規約・ポリシー**: 表記統一（神戸大学留学生支援サークル Truss / Truss公式アプリ）→ CMS 化（041）→ 保護・履歴・告知（042）→ 条文別エディタ + 公開ページのカード区切り
6. **運営画面**: 設定タブ新設（通知設定・規約管理を集約）/ チャット一覧の属性バッジ（承認待ち・交換留学・正規留学・非会員）
7. **役職**: 在任履歴 + 代表/副代表の1人制約と引き継ぎフロー（043）/ SE 役職（045、平石アカウント専用・SQL のみ）
8. **学年混在対策**: 年度ごとの本人確認ナッジ（044）+ 学籍番号推測による運営向け「学年要確認」バッジ

### 次セッションでの確認・残タスク
- [ ] migration 044 / 045 の本番適用状況の確認（CHECK_SCHEMA.sql で確認可。041〜043 は適用済みと確認済み）
- [ ] デプロイ後の実機確認: 設定タブ・条文エディタ・承認通知・学年ナッジ・チャットバッジ
- [ ] **連絡先メールの確定**（年度ごとに作り替え。truss.kobe@gmail.com は現行ではない）→ 確定後、運営画面 設定タブ → 規約・ポリシーの第13条（両文書）を書き換えて保存（コード変更不要）
- [ ] Google ブランド再審査の結果待ち → 通過後: 会員周知 → 旧 vercel.app → 新ドメインのリダイレクト有効化（周知と同時）
...
## 2026-09-04 00:40 引き継ぎの権限設計についての確認（コード変更なし）

「引き継ぎは代表だけか」「承認フローになっているのか」という質問への回答。実装の変更はしていない。

### 現状の事実（コードで確認）
- **承認フローは存在しない**。`transfer_role` は1トランザクションで即完了し、後任への通知も保留状態も無い
- 引き継ぎフローが走るのは **代表・副代表の2つだけ**（DB に1人制約があるのがこの2つ。役職者・顧問は複数可なので通常の役職変更）
- 実行できるのは `is_admin_safe()` が真の人**全員** = 役職者・副代表・代表・顧問教員・SE。代表本人である必要はなく、**役職者が単独で代表を挿げ替えられる**
- 「自分の役職は変更できない」ガード（isSelf）は役職セレクトにしか掛かっておらず、引き継ぎフローは後任の行を対象にするため前任が自分でも素通りする。本人も代理もできる建て付けで、本人だけが 046 のバグで落ちていた
- 運営メンバーの追加/削除は「役職を役職者にする / 部員に戻す」で行う（is_admin は役職に完全連動。039/045）

### ユーザーへ提示した選択肢（回答待ち）
1. **推奨**: 046 に `transfer_role` の実行権限を `is_admin_safe()` → `is_senior_admin_safe()`（代表・副代表・顧問・SE）へ絞る変更を追加。042 の保護文書と同じ線で揃う。046 未適用の今ならマイグレーション1本にまとめられる
2. 引き継ぎ完了時に新旧両者へ運営名義の通知を送る（承認は不要だが「知らないうちに外された」を防ぐ）
3. 現状のまま（046 の自滅修正だけ適用）

承認フロー自体は非推奨と回答。年1回・対面で確定する運用に対して、「承認待ちの代表」状態は1人制約と運営権限連動を不必要に複雑にするため。

### 残課題
- [ ] 上記1・2をやるかの判断（やるなら 046 適用前が望ましい）
- [ ] 前エントリの残課題（046 の本番適用と引き継ぎ再実行）は未着手のまま

## 2026-09-04 01:10 引き継ぎを「現職本人のみ」に限定 + 後任への通知（migration 047）

046 は本番適用済み。権限設計の判断を受けて 047 を新規追加。

### 決定内容
- 引き継ぎを実行できるのは **その役職の現職本人だけ**。運営権限があるだけでは不可（役職者が単独で代表を挿げ替えられる状態を解消）
- 代表が副代表を付け替える等の代理操作も認めない。顧問はアカウント未作成、SE は DB を直接操作できるため、上位役職による代理実行の口は設けない
- 承認フローは入れない。代わりに **引き継ぎ完了を後任へ運営名義メッセージで通知**する

### 変更内容
- `supabase/migrations/047_truss_role_transfer_holder_only.sql`（新規・**要適用**）
  - `transfer_role` の入口を `is_admin_safe()` → **呼び出し元の `users.role` が `p_role` と一致すること**に変更（`Only the current holder can transfer this role`）
  - 後任が自分自身のケースを弾く
  - 引数 `p_notice` を追加し、後任への運営名義メッセージを**同一トランザクションで INSERT**。クライアントから送ると、前任が「部員」に降りた直後は `messages_insert_approved_or_admin`（`is_admin_safe()` を要求）で弾かれるため。文面は i18n のあるアプリ側が渡す
  - 引数の数が変わるため旧シグネチャを `DROP FUNCTION` してから作り直す（PostgREST のオーバーロード曖昧化を避ける）
  - SE / 開発者が transfer_role を使わず直接付け替える手順をコメントに明記（Dashboard は `auth.role()` が `authenticated` でないので 032 のガードを通る）
- `packages/core/src/types/database.types.ts` / `db/mutations/role-history.ts`: `p_notice` を追加
- `packages/core/src/db/errors.ts`: 失敗理由に `not-role-holder` を追加
- `apps/web/src/contexts/DataContext.tsx`: `transferRole` に `notice` を通す
- `MemberDetailModal.tsx`:
  - 現職本人でなければ**確認ダイアログを開く前に**「現職の◯◯さん本人からのみ行えます」と案内（押してから断られない）
  - 前任は常に操作者本人になったため `holderId` を廃し、自己降格の警告は「引き継ぎ後＝部員」のときだけ表示
  - 後任への通知文（日英併記）を組み立てて `transferRole` に渡す
- `supabase/DIAGNOSE_ROLE_TRANSFER.sql`: `Only the current holder ...` の項と 047 の適用確認（セクション7）を追加

### 検証
- `tsc --noEmit`（web / core）通過、`next build` 通過、lint はベースラインと同一（99 problems、新規指摘なし）
- 047 は**未適用**

### 残課題
- [ ] **migration 047 を本番に適用** → 現代表のアカウントで引き継ぎを実行し、成功と後任への通知到達を確認
- [ ] 適用後、`DIAGNOSE_ROLE_TRANSFER.sql` のセクション6・7が OK になることを確認
- [ ] 後任へのプッシュ通知は未対応（アプリ内メッセージのみ）。前任が降格直後で運営権限を失うため、`/api/push/send` を叩けないのが理由。必要なら送信をサーバー側へ寄せる
