...
- **リポジトリを `~/Developer/truss-app` へ移行済み**。理由: 旧パス `~/Documents/Kobe-u/Truss/truss-app` は「デスクトップと書類」iCloud同期の対象で、`fileproviderd`が高負荷になり`git mv`/`rsync`/`ditto`いずれも実用にならないほど遅かった（node_modules除外後も進まず）。最終的に `git clone` + `.env.local`/`.vercel/`/`.claude/`/未コミットの`docs/tasks.md`を個別コピーする方式で移行。
- 新しい場所(`~/Developer/truss-app`)で `npm install`(8秒)/`npm run build`/`npm run dev`（HTTP 200確認）まで動作確認済み。`git log`は旧パスと同一コミット（`2a65c44`）、`git status`もクリーン。
- **旧パスはまだ削除していない**。理由: このセッションを動かしているCursorウィンドウ自体が旧パスをワークスペースとして開いたままだったため（削除するとCursor側が壊れる）。
- **次にやること**:
  1. Cursorで `~/Developer/truss-app` を `File > Open Folder` で開き直す
  2. 新しいウィンドウで `claude` を起動し、新セッションを開始（このメモを読めば経緯が分かる）
  3. 旧パス `~/Documents/Kobe-u/Truss/truss-app` を削除
  4. Phase 1（モノレポ化）に着手
...
- [ ] `git mv` で `src/`, `public/`, `next.config.ts`, `package.json`, `tsconfig.json`, `postcss.config.mjs`, `eslint.config.mjs`, `.env.local` 等を `apps/web/` へ移動（履歴を保持）
- [ ] ルートに workspaces 用 `package.json`（`"workspaces": ["apps/*", "packages/*"]`）を新設
- [ ] ルート `next-env.d.ts` / `.next` / `tsconfig.tsbuildinfo` が `apps/web/` 側で再生成される前提で `.gitignore` を確認
- [ ] ローカルで `npm install` → `npm run build` → `npm run dev` が `apps/web` で問題なく通ることを確認
- [ ] Vercel Project Settings → Root Directory を `apps/web` に変更（Preview環境で先に確認してから本番反映）
- [ ] `AGENTS.md`（Next.js breaking changes注意書き）を `apps/web/AGENTS.md` に複製、ルートAGENTS.mdはモノレポ全体向けに書き換え
- [ ] 本番マージ・デプロイ確認
...
- [ ] **`AuthContext.tsx` の直接Supabase呼び出しを `lib/db/queries|mutations` に切り出す**（現状 `supabase.auth.*` に加え `supabase.from('users')...` への直接クエリが混在し、他のドメインロジックと分離レベルが揃っていない）
- [ ] **未使用の `src/hooks/useSupabase.ts`（1198行）の要否を判断**（`useCurrentUser`/`useEvents`/`useMessages`等を定義しているが、どこからもimportされていないデッドコード。DataContextへの移行前の旧実装と思われる。削除するか、含まれるロジックで再利用すべきものがないか確認してから判断）
- [ ] `lib/db/queries/` の手薄さを解消（現状 `fee-settings.ts`/`public-reads.ts` のみ。`mutations/`側8ファイルに比べクエリ側の整理が追いついていない）
- [ ] `src/domain/types/app.ts` → ドメイン型を移動
- [ ] `src/lib/supabase.ts` をファクトリ化し、`storage`（localStorage/AsyncStorage）と `redirectTo` を呼び出し側から注入可能にする
- [ ] `src/lib/db/queries/*`, `src/lib/db/mutations/*` を移動
- [ ] `src/lib/profile-completion.ts`, `event-icons.ts`, `board-content.ts`, `event-map-link.ts` 等の純粋ロジックを移動
- [ ] `database.types.ts`（Supabase生成型）を移動
- [ ] `apps/web` からの参照をpackages/core経由に置き換え
- [ ] 既存テスト（vitest等があれば）が通ることを確認
