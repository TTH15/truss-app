...
plan.md 7節の元案は`chat_threads`テーブルによる本格的なマルチスレッド化（1メンバー×1カテゴリ＝1スレッド、運営が問い合わせごとにopen/in_progress/resolvedを管理）だったが、実装着手前に再確認した結果、**軽量版**（今の「1メンバー=1会話」のシンプルな構造は変えず、各メッセージに`category`タグだけ付ける）を採用することで合意。`chat_threads`テーブルは見送り。

- [x] **相談カテゴリ分け** — `messages.category`列（`inquiry`/`event_consult`/`membership`/`trouble`のenum）。会員がメッセージ送信時にカテゴリチップで選択（モバイル: `TrussEmbassyScreen.tsx`、Web: `MessagesPage.tsx`）。運営側は選択不要（表示・参照のみ、`AdminChatMessages.tsx`）。ラベル定義は`packages/core/src/message-categories.ts`で共有
- [x] **既読表示（双方向）** — 既存の`read`（boolean）は残し`read_at`タイムスタンプを追加。会員→運営方向の既読は元々DB永続化されていなかった（`AdminChatMessages.tsx`の既読処理がローカルstateのみで完結していた）ため、新設の`markMemberMessagesAsReadRow`で追加。RLSの`messages_update_receiver_or_admin`は元から双方向に対応済みだったためポリシー変更は不要
- [x] **画像添付**（V1は画像のみ、汎用ファイルは見送り） — `chat-attachments` Storageバケット新設（非公開、`{userId}/{timestamp}-{suffix}.{ext}`のパス）。RLS SELECTは`messages`テーブルへの参照付きポリシーで、そのメッセージの送信者・受信者本人と管理者のみ閲覧可（gallery-photos等の「承認済みなら誰でも」より狭い、チャット内容の非公開性を優先）。表示は署名付きURL（`getChatAttachmentSignedUrl`）を都度発行
- [ ] Expo Push Notifications + Supabase Edge Function（`messages` insertトリガー）— 未着手。Expo Push自体はExpo Goでも動作するため、Dev Client移行（Phase 5）を待たずに着手可能
- **マイグレーション**: `supabase/migrations/026_truss_chat_enhancement.sql`。dev DBに適用・動作確認済み。**本番適用はユーザーがDashboard SQL Editorから手動実施予定（未実施）**
- **未実装**: 添付ファイルの削除UI（送信取り消し等）、カテゴリによる会話の絞り込み表示（運営側一覧でのフィルタ）
- **未検証**: 実データでのカテゴリ・既読・添付フロー全体（dev DBでのE2Eは未実施、UIレンダリングはモックデータ注入で確認済み）
...
## 次セッションへの引き継ぎ（2026-08-05 時点）

Web 側は一区切り。次はモバイル（Phase 3）へ移る想定。以下は Web に残っている宿題。

### 本番 DB への適用が必要（優先）

- [ ] マイグレーション **035**（`non_member` ロール + 会費支払いでの自動昇格）を適用
- [ ] マイグレーション **036**（退会・内部データ保持）を適用
- [ ] マイグレーション **037**（通知種別ごとの受信設定）を適用
- [ ] **031**（電話番号のハイフン除去）が適用済みか未確認。データのみの変更なのでスキーマ監査では検出できない。`supabase/CHECK_SCHEMA.sql` 末尾の電話番号チェックで確認する
- 034 は適用済みを確認済み。033 は欠番（作成後に削除。`gallery_photo_likes` は 001 に既にあった）

### 実装が途中のもの

- [ ] 一斉送信・イベント案内から `sendPushNotification()` を呼ぶ（`category: 'announcement' | 'event'` を渡すだけ。個別メッセージからの送信は接続済み）
- [ ] 初回登録時に `find_withdrawn_record()` を呼んで会費の支払い状況を引き継ぐ（関数は 036 で作成済み、呼び出し側が未実装。**未払いのまま作り直して逃げられる穴**が塞がっていない）
- [ ] チャット履歴のスレッドごとのページング。現状は新しい方から 1000 件で頭打ち（PostgREST の `db-max-rows` に当たっていたのを、降順 + limit + 反転で暫定回避している）
- [ ] Web Push の実機確認（iOS はホーム画面に追加してから）

### 保留

- Journey Stamp は画面から外してある。電子スタンプ（NFC/BLE）の方針が決まってから戻す（Phase 5 に復帰手順を記載）
- 一斉送信のメール通知は無効化中（送信基盤がないため。アプリ通知のみ）
