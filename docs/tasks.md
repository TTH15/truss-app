...
- **Phase 4の主要画面（プロフィール登録フォーム・Passport・Journey・Truss Embassy・Memories・Connections）実装完了**。詳細はPhase 4セクション参照。
- **次にやること**: QRコードイベントチェックイン（MVP必須機能、Phase 4セクション内）、または実データでのE2E動作確認（各画面「未検証」項目参照）。
...
- [ ] チケット画面裏面（タップで裏返り）にQRコードを表示
- [ ] QRペイロード設計（`{eventId, userId}` + 署名等の軽量方式。`event_participants`への新規カラム追加なしでの実現を優先検討）
- [ ] 運営側スキャナー画面（カメラでQR読み取り → `event_participants.attended` を確定）
