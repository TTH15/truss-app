...
## 次セッションへの引き継ぎ（2026-08-07 更新）

### 本番 DB への適用（完了）

- [x] マイグレーション 035・036・037 適用済み（`CHECK_SCHEMA.sql` で確認）
- [x] 031（電話番号のハイフン除去）確認済み（`phone_not_normalized: 0`）
- 034 は適用済み。033 は欠番（作成後に削除。`gallery_photo_likes` は 001 に既にあった）
...
- モバイルのイベントいいね: `toggleEventLike` はトグルだが `likedEventIds` を画面が持たず、どれをいいね済みか分からない（ギャラリーは 2026-08-06 に対応済み。同じ形で直せる）
- Web の初期登録の書き込みが LegacyApp 直書きと core の `completeInitialRegistrationRow` に二重にある（モバイルは core 経由）。Web も core に寄せたい
